...
class MainViewModel: ViewModel() {
            ...
            try {
                Socket(printerIp, port).use { socket ->
                    ...
                    socket.getOutputStream().use { outStream ->
                        contentResolver.openInputStream(pdfUri)?.use { inStream ->
    ...
    fun generatePdfFromFields(context: Context,
        discountPercentage:Int, productName:String, initialPriceCents: Int, finalPriceCents: Int, qrCodeInt: String) {
        ...
        canvas.drawText(qrCodeInt, qrRect.centerX(), qrRect.bottom + 6f, qrTextPaint)
